"""data/datasets — PyPI ``datasets`` (Hugging Face Datasets).
...
class DatasetsDataProvider:
    ...
    def list_datasets(self) -> list[str]:
        return ['openai/gsm8k', 'cais/mmlu', 'glue', 'imdb', 'squad']
    ...
    def build(
        ...
        **kwargs,
        ...
        aliases = {'GSM8K': ('openai/gsm8k', 'main'), 'gsm8k': ('openai/gsm8k', 'main')}
        ...
        if 'train' not in out:
            raise ValueError(f'datasets {data_name} missing train; have {list(ds.keys())}')
        if 'test' not in out:
            out['test'] = out['train']
        if verbose:
